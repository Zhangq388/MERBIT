...
struct TILE1
{
    ...
    int                      lane_num;
    TILE1(){};
    TILE1(const int* __restrict__ offset, const int nrow, const int nnz, const int L);
    void show();
...
void tile1_kernel(const int* __restrict__            offset, 
                  ...
                  DescTypeT<SIGMA>* __restrict__     lane_desc)
{
    ...
    if(gid < lane_num)
    {
        bool fast = tile_y[tile_id] >> 31;
        unsigned int local_tile_y = (tile_y[tile_id] << 1) >> 1;
        ...
        if (!fast)
        {
            ...
            int diag = tile_x[tile_id] + local_tile_y + lid * SIGMA;
            int lane_nnz = (diag + SIGMA < nnz + nrow)? SIGMA : (nnz + nrow - diag);
            int coord_x, coord_y;
            ...
            DescType delta_x = coord_x - tile_x[tile_id]; //delta_x
            DescType delta_y = coord_y - local_tile_y; //delta_y
...
template<unsigned int BLOCK, unsigned int SIGMA>
TILE1<BLOCK, SIGMA>::TILE1(const int* __restrict__ offset, 
                           const int               nrow, 
                           const int               nnz,
                           const int               L)
{
    ...
    //变量定义并申请空间
    int* h_offset = (int*)malloc(sizeof(int) * (nrow + 1));
    cudaMemcpy(h_offset, offset, sizeof(int) * (nrow + 1), cudaMemcpyDeviceToHost);
    //SHOW(h_offset, 104492304, 3);

    unsigned int* h_tile_x = (unsigned int*)calloc(nrow + nnz, sizeof(unsigned int)); //实际上用不完
    unsigned int* h_tile_y = (unsigned int*)calloc(nrow + nnz, sizeof(unsigned int));
    unsigned int* h_tile_s = (unsigned int*)calloc(nrow + nnz, sizeof(unsigned int)); 
    
    const unsigned int warpnum = BLOCK / 32;
    int diag(0), coord_x0(0), coord_y0(0), coord_x1(0), coord_y1(0), idx(0), sum(0);
    while (diag < nrow + nnz)
    {
        bool boundary = (idx % warpnum == 0); //BLOCK起始tile
        
        //compact the block_x_nnz 与 warp_block_coord_x_start
        if (boundary && idx > 0)
        {
          h_tile_s[(idx - 1) / warpnum * warpnum] = (sum << 16) | h_tile_s[(idx - 1) / warpnum * warpnum];
          sum = 0;
        }
        
        if (h_offset[coord_y0 + 1] - coord_x0 < 128)
        {
            merge_search<SIGMA>(h_offset, nrow, nnz, diag + L, coord_x1, coord_y1);
            h_tile_x[idx] = coord_x0;
            h_tile_y[idx] = coord_y0;
            h_tile_s[idx] = sum;
            sum += coord_x1 - coord_x0;

            coord_x0 = coord_x1;
            coord_y0 = coord_y1;
            diag = diag + L;
        }
        else if (h_offset[coord_y0 + 1] - coord_x0 >= 128 && h_offset[coord_y0 + 1] - coord_x0 <= 1024)
        {
            h_tile_x[idx] = coord_x0;
            h_tile_y[idx] = coord_y0 | 0x80000000;
            h_tile_s[idx] = sum;

            diag = diag + h_offset[coord_y0 + 1] - coord_x0 + 1;
            coord_x0 = h_offset[coord_y0 + 1];
            coord_y0 = coord_y0 + 1;
        }
        else
        {
    ...
    //计算tile_num和lane_num
    tile_num = idx;
    lane_num = (tile_num - 1) * 32 + ((h_tile_y[tile_num - 1] >> 31)? 0 : ((nrow + nnz - h_tile_x[tile_num - 1] - h_tile_y[tile_num - 1] + SIGMA - 1) / SIGMA));

    cudaMalloc((void**)&tile_x, sizeof(unsigned int) * (tile_num + 1));
    cudaMalloc((void**)&tile_y, sizeof(unsigned int) * (tile_num + 1));
    cudaMalloc((void**)&tile_s, sizeof(unsigned int) * (tile_num + 1));
    cudaMalloc((void**)&lane_desc, sizeof(DescType) * lane_num);

    cudaMemcpy(tile_x, h_tile_x, sizeof(unsigned int) * (tile_num + 1), cudaMemcpyHostToDevice);
    cudaMemcpy(tile_y, h_tile_y, sizeof(unsigned int) * (tile_num + 1), cudaMemcpyHostToDevice);
    cudaMemcpy(tile_s, h_tile_s, sizeof(unsigned int) * (tile_num + 1), cudaMemcpyHostToDevice);
    
    int grid = (lane_num + BLOCK -1) / BLOCK;
    tile1_kernel<BLOCK, SIGMA><<<grid, BLOCK>>>(offset, tile_x, tile_y, nrow, nnz, tile_num, lane_num, lane_desc);
    
    //CUDASHOW(tile_x, unsigned int, 16);
    //CUDASHOW(&tile_x[tile_num-1], unsigned int, 2);
    //CUDASHOW(tile_y, unsigned int, 16);
    //CUDASHOW(&tile_y[tile_num-1], unsigned int, 2);
...
void TILE1<BLOCK, SIGMA>::show()
{
    ...
    std::cout << "tile_s: " << "\n";
    unsigned int h_tile_s[tile_num + 1];
    cudaMemcpy(h_tile_s, tile_s, sizeof(unsigned int) * (tile_num + 1), cudaMemcpyDeviceToHost);
    for(int idx=0; idx<tile_num + 1; ++idx)
    {
        int block_x_nnz = h_tile_s[idx] >> 16;
        int warp_block_coord_x_start = (h_tile_s[idx] << 16) >> 16;
        std::cout << block_x_nnz << "|" << warp_block_coord_x_start << " ";
    }
    std::cout << "\n";
...
//desc
template<unsigned int BLOCK, unsigned int SIGMA> __global__ 
void tile1_desc_kernel(const unsigned int* __restrict__ tile_x, 
                       const unsigned int* __restrict__ tile_y, 
                       const int                        tile_num, 
                       int*                             tile_fast_num,
                       int*                             tile_fast_nnz)
{
    ...
    if(tile_id < tile_num)
    {
        ...
        if(tile_fast) 
        {
            ...
            atomicAdd(tile_fast_nnz, warp_coord_x_end - warp_coord_x_start);
        }
    }
}

template<unsigned int BLOCK, unsigned int SIGMA>
void TILE1<BLOCK, SIGMA>::desc()
{
    int* d_tile_fast_num;
    int* d_tile_fast_nnz;
    cudaMalloc((void**)&d_tile_fast_num, sizeof(int));
    cudaMalloc((void**)&d_tile_fast_nnz, sizeof(int));
    cudaMemset(d_tile_fast_num, 0, sizeof(int));
    cudaMemset(d_tile_fast_nnz, 0, sizeof(int));

    const int grid = (tile_num + BLOCK - 1) / BLOCK;
    tile1_desc_kernel<BLOCK, SIGMA><<<grid, BLOCK>>>(tile_x, tile_y, tile_num, d_tile_fast_num, d_tile_fast_nnz);

    int tile_fast_num, tile_fast_nnz;
    cudaMemcpy(&tile_fast_num, d_tile_fast_num, sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(&tile_fast_nnz, d_tile_fast_nnz, sizeof(int), cudaMemcpyDeviceToHost);
    CUDAFREE(d_tile_fast_num);
    CUDAFREE(d_tile_fast_nnz);
    ...
    std::cout << ", sigma = " << SIGMA << ", tile_num = " << tile_num << ", tile_fast_num = " << tile_fast_num << ", tile_fast_nzz = " << tile_fast_nnz << "\n";
}

//fast_track
template<typename ValueType> __device__ 
void tile1_fast_track(const int* __restrict__       row_ind, 
...
void tile1_load_smem(const int* __restrict__       row_ind, 
                     const ValueType* __restrict__ values, 
                     const int                     warp_x_nnz, 
                     const int                     lid, 
                     const ValueType* __restrict__ d_x, 
...
void tile1_normal_track(const ValueType* __restrict__ s_data, 
...
template<typename ValueType> __device__
void tile1_load_mem(const ValueType* __restrict__ s_y, 
...
void merbit1_kernel(const int* __restrict__                row_ind, 
                    const ValueType* __restrict__          values, 
                    const int                              nrow, 
                    const int                              nnz, 
                    const unsigned int* __restrict__       tile_x, 
                    const unsigned int* __restrict__       tile_y, 
                    const unsigned int* __restrict__       tile_s, 
                    ...
                    ValueType* __restrict__                d_z)
{
    ...
    int block_x_nnz = tile_s[block_tile_start] >> 16;
    int warp_block_coord_x_start = (tile_s[tile_id] << 16) >> 16;
    ...
    if(tile_id<tile_num)
    {
        ...
        if(warp_coord_x_start < warp_coord_x_end)
        {
            ...
            if(fast)
            {
                tile1_fast_track<ValueType>(&row_ind[warp_coord_x_start], &values[warp_coord_x_start], warp_x_nnz, lid, d_x, &s_y[warp_block_coord_y_start]);
            }
            else
            {
                //加载至共享内存
                tile1_load_smem<ValueType>(&row_ind[warp_coord_x_start], &values[warp_coord_x_start], warp_x_nnz, lid, d_x, &s_data[warp_block_coord_x_start]);

                //开始处理
                if(gid < lane_num)
                {   
                    int warp_len = ((tile_id + 1) * 32 < lane_num)? 32 : (lane_num - tile_id * 32);
                    int lane_nnz = (warp_coord_x_start + warp_coord_y_start + (lid + 1) * SIGMA < nnz + nrow)? SIGMA : (nnz + nrow - warp_coord_x_start - warp_coord_y_start - lid * SIGMA);
                    DescType local_lane_desc = lane_desc[gid];
                    tile1_normal_track<ValueType, SIGMA>(&s_data[warp_block_coord_x_start], local_lane_desc, warp_len, lane_nnz, lid, &s_y[warp_block_coord_y_start]);
                }
            }
        }
    }
    ...
    //将数据从共享内存加载到主存
    tile1_load_mem<ValueType>(s_y, block_y_nnz, tid, BLOCK, alpha, &d_y[block_coord_y_start], &d_z[block_coord_y_start]);
}

template<typename ValueType, unsigned int BLOCK, unsigned int SIGMA> 
void Fun_Merbit1(const ENV& env, const PARAMETER<ValueType>& para)
{
    //变量赋值
    Timer   timer;
    ...
    TILE1<BLOCK, SIGMA> tile(offset, para.graph->nrow, para.graph->nnz, 32 * SIGMA);
    //tile.show();
    //tile.desc();
    ...
    {
        merbit1_kernel<ValueType, BLOCK, SIGMA><<<grid, BLOCK, 0, env.stream>>>(para.graph->row_ind, para.graph->values, para.graph->nrow, para.graph->nnz, tile.tile_x, tile.tile_y, tile.tile_s, tile.lane_desc, tile.tile_num, tile.lane_num, 1.0, para.person, d_y, d_z);
        std::swap(d_y, d_z);
    }
