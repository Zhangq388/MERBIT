...
template<typename IndexType, typename uiT> __global__
void generate_partition_pointer_s1_kernel(const IndexType*     d_row_pointer,
...
{
    // global thread id
    IndexType global_id = blockIdx.x * blockDim.x + threadIdx.x; //get_global_id(0);

    // compute partition boundaries by partition of size sigma * omega
    IndexType boundary = global_id * sigma * ANONYMOUSLIB_CSR5_OMEGA;
    ...
    if (global_id <= p)
    {
         d_partition_pointer[global_id] = cub::UpperBound(d_row_pointer, m + 1, boundary) - 1;
    }
}


template<typename IndexType, typename uiT> __global__
void generate_partition_pointer_s2_kernel2(const IndexType   *d_row_pointer,
                                           uiT        *d_partition_pointer)
{
    //生成指标
    const IndexType bid = blockIdx.x;
    const IndexType tid = threadIdx.x;
    const IndexType wid = tid >> 5;
    const IndexType lid = tid & 31;
    volatile __shared__ bool is_empty;
    ...
    //确定起点终点
    uiT start = d_partition_pointer[bid];
    uiT stop  = d_partition_pointer[bid+1];
    start = (start << 1) >> 1;
    stop  = (stop << 1) >> 1;
    if(start == stop)
    {
    ...
    uiT nnz = stop + 1 - start;

    if(tid < nnz)
    {
        //读取数据
        uiT data = d_row_pointer[start + tid];

        //同一个warp内
        uiT result = data - __shfl_up_sync(0xFFFFFFFF, data, 1);
        
        //warp间
        if(wid && !lid)
        {
            result = data - d_row_pointer[start + tid - 1];
        }
        ...
        if(!tid && is_empty)
        {
            ...
            d_partition_pointer[bid] = start;
        }
    }
}

template<typename IndexType, typename uiT> __global__
void generate_partition_pointer_s2_kernel(const IndexType   *d_row_pointer,
                                          uiT        *d_partition_pointer)
{
    ...
    IndexType row_idx, row_off_l, row_off_r;
...
template<typename IndexType, typename uiT> __global__
void generate_partition_descriptor_s1_kernel(const IndexType    *d_row_pointer,
                                             ...
                                             const int           num_packet)
{
    ...
    if (global_id < m)
    {
        const IndexType  row_offset = d_row_pointer[global_id];
        const IndexType  gx    = row_offset / sigma;
        const IndexType  lx    = gx % ANONYMOUSLIB_CSR5_OMEGA;
        const IndexType  pid   = gx / ANONYMOUSLIB_CSR5_OMEGA;
        ...
        const int location = pid * ANONYMOUSLIB_CSR5_OMEGA * num_packet + ly * ANONYMOUSLIB_CSR5_OMEGA + lx;//横排的
...
void generate_partition_descriptor_s2_kernel(const uiT    *d_partition_pointer,
                                             uiT          *d_partition_descriptor,
                                             IndexType    *d_partition_descriptor_offset_pointer,
                                             const int     sigma,
                                             const int     num_packet,
                                             ...
                                             const int     p)
{
    ...
    const IndexType row_stop   = s_row_start_stop[bunch_id + 1] & 0x7FFFFFFF;
...
void generate_partition_descriptor_s3_kernel(IndexType    *d_partition_descriptor_offset_pointer,
                                             const int     p)
{
    const int local_id = threadIdx.x;
    ...
    IndexType sum = 0;
    volatile __shared__ IndexType s_partition_descriptor_offset_pointer[256 + 1];
    ...
    for (int i = 0; i < loop; i++)
    {
        ...
        scan_256_plus1<IndexType>(s_partition_descriptor_offset_pointer);
        __syncthreads();
...
{
    //s1每个线程处理原始矩阵的一行,bit_flag打每一行开始的编号
    int num_threads = 128;
    ...
    generate_partition_descriptor_s2_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT><<< num_blocks, num_threads >>>(partition_pointer, partition_descriptor, partition_descriptor_offset_pointer, sigma, num_packet, bit_y_offset, bit_scansum_offset, p);
    ...
    if (num_offsets)
    {
        ...
        num_threads = 256;
        num_blocks  = 1;
        generate_partition_descriptor_s3_kernel<ANONYMOUSLIB_IT><<< num_blocks, num_threads >>>(partition_descriptor_offset_pointer, p);
        cudaMemcpy(&num_offsets, &partition_descriptor_offset_pointer[p], sizeof(ANONYMOUSLIB_IT),   cudaMemcpyDeviceToHost);
    }
...
template<typename IndexType, typename uiT> __inline__ __device__
void partition_normal_track_empty_Ologn(const IndexType      *d_row_pointer,
                                        const uiT            *d_partition_descriptor,
                                        const IndexType      *d_partition_descriptor_offset_pointer,
                                        IndexType            *d_partition_descriptor_offset,
                                        const IndexType       par_id,
                                        const int             lane_id,
                                        const int             bit_y_offset,
                                        const int             bit_scansum_offset,
                                        IndexType             row_start,
                                        const IndexType       row_stop,
                                        const int             c_sigma)
{
    bool local_bit;
    ...
    {
        const IndexType idx = par_id * ANONYMOUSLIB_CSR5_OMEGA * c_sigma + lane_id * c_sigma;
        const IndexType y_index = cub::UpperBound(&d_row_pointer[row_start+1], row_stop - row_start, idx) - 1;
    ...
    for (int i = 1; i < c_sigma; i++)
    {
        ...
        if (local_bit)
        {
            const IndexType idx = par_id * ANONYMOUSLIB_CSR5_OMEGA * c_sigma + lane_id * c_sigma + i;
            const IndexType y_index = cub::UpperBound(&d_row_pointer[row_start+1], row_stop - row_start, idx) - 1;
            d_partition_descriptor_offset[offset_pointer + y_offset] = y_index;
...
template<typename IndexType, typename uiT> __inline__ __device__
void generate_partition_descriptor_offset_partition(const IndexType    *d_row_pointer,
                                                    ...
                                                    const int           c_sigma)
{
    ...
    if (row_start >> 31) // with empty rows
    {
        ...
        partition_normal_track_empty_Ologn<IndexType, uiT>(d_row_pointer, d_partition_descriptor, d_partition_descriptor_offset_pointer, d_partition_descriptor_offset, par_id, lane_id, bit_y_offset, bit_scansum_offset, row_start, row_stop, c_sigma);
    }
...
template<typename iT, typename uiT> __global__
                                         ...
                                         ANONYMOUSLIB_IT        *partition_descriptor_offset)
{
    ...
    generate_partition_descriptor_offset_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT><<< num_blocks, num_threads >>>(row_pointer, partition_pointer, partition_descriptor, partition_descriptor_offset_pointer, partition_descriptor_offset, p, num_packet, bit_y_offset, bit_scansum_offset, sigma);
    return ANONYMOUSLIB_SUCCESS;
}

template<typename T, typename uiT, int CSR5_SIGMA> __global__
