...
template<typename T> struct dCSR
{
  ...
  dCSR(const size_t nrow, const size_t ncol, const size_t nnz, T* values, int* offset, int* col_ind) : rows(nrow), cols(ncol), nnz(nnz)
	{ 
		data = values;
		row_offsets = (unsigned int*)offset;
		col_ids = (unsigned int*)col_ind;
	};
	~dCSR(){ };
};

template<typename T> struct dDenseVector
{
	size_t size;
	T* data;

	dDenseVector() : size(0), data(nullptr) { };
  dDenseVector(const size_t nrow, T* values) : size(nrow), data(values) { };
	dDenseVector(const size_t nrow) 
	{ 
		size = nrow;
...
private:
	template<class OFFSET_TYPE> __device__ __forceinline__ 
	uint32_t runSimple(SMem& smem, FlagSMem& flagmem, uint32_t blockRowStart, uint32_t blockNumRows, const OFFSET_TYPE* __restrict offsets)
	{
		uint32_t flag = 0;
		...
		if (BLOCK_FLAG)
		{
			if (flag)
			{
				uint32_t f_other = __shfl_sync(0xFFFFFFFF,flag, 0);
				bool diff = __any_sync(0xFFFFFFFF, f_other != flag);
				if (laneid() == 0)
				{
	...
	template<class OFFSET_TYPE> __device__ __forceinline__ 
	void runThread(SMem& smem, FlagSMem& flagmem, uint32_t blockRowStart, uint32_t blockNumRows, const OFFSET_TYPE* __restrict offsets)
	{
		for (uint32_t r = threadIdx.x; r < blockNumRows; r += THREADS)
		{
	...
	template<class OFFSET_TYPE> __device__ __forceinline__ 
	void runWarp(SMem& smem, FlagSMem& flagmem, uint32_t blockRowStart, uint32_t blockNumRows, const OFFSET_TYPE* __restrict offsets)
	{
...
public:
	template<class OFFSET_TYPE> __device__ __forceinline__ 
	void initSingle(SMem& smem, FlagSMem& flagsmem, uint32_t blockRowStart, uint32_t blockNumRows, const OFFSET_TYPE* __restrict offsets)
	{
	...
	template<class OFFSET_TYPE> __device__ __forceinline__ 
	uint32_t run(SMem& smem, FlagSMem& flagmem, uint32_t blockRowStart, uint32_t blockNumRows, const OFFSET_TYPE* __restrict offsets)
	{
...
struct OutputBuffer
{
	...
	__device__ __forceinline__ void writeBuffer(VALUE_TYPE3* __restrict outVec, COUNTER_SMEM& counterSmem, bool last)
	{
		...
		if (threadIdx.x == 0 || last)
		{
			...
			atomicAdd(outVec + counterSmem.bStart + address, buffer[address]);
		}
	}
...
struct ValueLoader<THREADS, NNZ_PER_THREAD, true, VALUE_TYPE1, VALUE_TYPE2, VALUE_TYPE3, INDEX_TYPE, 0>
{
	...
	__device__ __forceinline__ static void syncSmem() {}

	__device__ __forceinline__ static void loadAndPremultiply(SMem& smem, VALUE_TYPE3(&values)[NNZ_PER_THREAD], uint32_t nnz, const VALUE_TYPE1* matrix, const INDEX_TYPE* inIndex, const VALUE_TYPE2* __restrict inVec)
	{
	...
	__device__ static void loadWithIndices(SMem& smem, VALUE_TYPE3(&values)[NNZ_PER_THREAD], INDEX_TYPE(&indices)[NNZ_PER_THREAD], const VALUE_TYPE1* matrix, const INDEX_TYPE* inIndex)
	{
		const int MaxVecLoadIndex = 16 / sizeof(INDEX_TYPE);
		const int MaxVecLoadMat = 16 / sizeof(VALUE_TYPE1);
	...
	__device__ static void loadValues(VALUE_TYPE3(&values)[NNZ_PER_THREAD], const VALUE_TYPE1* matrix, uint32_t nnz)
	{
		const int MaxVecLoadMat = 16 / sizeof(VALUE_TYPE1);
		warp_load_vectorized<MaxVecLoadMat>(values, matrix + blockIdx.x*(THREADS*NNZ_PER_THREAD));
		vectorized_to_blocked<MaxVecLoadMat>(values);
	...
	__device__ static void loadIndices(INDEX_TYPE(&indices)[NNZ_PER_THREAD], const INDEX_TYPE* inIndex, uint32_t nnz)
	{
		const int MaxVecLoadIndex = 16 / sizeof(INDEX_TYPE);
		warp_load_vectorized<MaxVecLoadIndex>(indices, inIndex + blockIdx.x*(THREADS*NNZ_PER_THREAD));
		vectorized_to_blocked<MaxVecLoadIndex>(indices);
...
template<class ThreadStarts, uint32_t THREADS, uint32_t NNZ_PER_THREAD, bool PADDEDLOAD, typename VALUE_TYPE1, typename VALUE_TYPE2, typename VALUE_TYPE3, typename INDEX_TYPE, typename OFFSET_TYPE, bool ZEROCHECK, bool SCALE, int SORTED_LOAD, class ToHandleOptimizations, class HandledOptimizations>
class Multiplication;


template<class ThreadStarts, uint32_t THREADS, uint32_t NNZ_PER_THREAD, bool PADDEDLOAD, typename VALUE_TYPE1, typename VALUE_TYPE2, typename VALUE_TYPE3, typename INDEX_TYPE, typename OFFSET_TYPE, bool ZEROCHECK, bool SCALE, int SORTED_LOAD, int HANDLED>
struct Multiplication<ThreadStarts, THREADS, NNZ_PER_THREAD, PADDEDLOAD, VALUE_TYPE1, VALUE_TYPE2, VALUE_TYPE3, INDEX_TYPE, OFFSET_TYPE, ZEROCHECK, SCALE, SORTED_LOAD, sequence<>, sequence<HANDLED>>
{
public:
	...
	template<class COUNTER_SMEM, class THREADINFO_SMEM> __device__ __forceinline__ static 
	void run(uint32_t flag, const VALUE_TYPE1* matrix, const INDEX_TYPE* inIndex, const OFFSET_TYPE* __restrict offsets, const VALUE_TYPE2* __restrict inVec, VALUE_TYPE3* __restrict outVec, VALUE_TYPE3 scale, uint32_t nnz, COUNTER_SMEM& counterSmem, THREADINFO_SMEM& threadInfoSmem, SMem& smem)
	{
		...
		{
			//we have got many zero rows, we cannot even buffer the output, so everything needs to be done dynamically
      ...
      #pragma unroll
		...
		else if (ZEROCHECK && counterSmem.flags.zerorow() != 0)
		{
			...
			uint32_t first_start = __shfl_sync(0xFFFFFFFF, my_row, 0);
      ...
      #pragma unroll
		...
		if (__popc(__ballot_sync(0xFFFFFFFF, my_bitmask)) < 16)
		{
      ...
      #pragma unroll
			...
			{
				VALUE_TYPE3 incoming_res = __shfl_down_sync(0xFFFFFFFF, pivot ? 0.0f : res, offset);
				if (!pivot_right)
					res += incoming_res;
				bool incoming_pivot = __shfl_down_sync(0xFFFFFFFF, pivot || pivot_right, offset);
				pivot_right = pivot_right || incoming_pivot;
			}
			if ((pivot || laneid() == 0) && res != 0)
			{
				atomicAdd(smem.outputBuffer.buffer + my_row, res);
			}
		}
		else
		{
		...
		else
		{
			...
			bool writelast = my_row != 1 && ((my_row == counterSmem.bNum && (threadIdx.x*NNZ_PER_THREAD < nnz_block)) || (static_cast<int>(nnz_block / NNZ_PER_THREAD) - 1 == -threadIdx.x));
			// && (threadIdx.x*NNZ_PER_THREAD < max(NNZ_PER_THREAD+1, (nnz - blockIdx.x*(THREADS*NNZ_PER_THREAD))));
      // && ((my_bitmask & (0x1 << NNZ_PER_THREAD - 1)) != 0);
			smem.outputBuffer.writeBuffer(outVec, counterSmem, writelast);
		}
...
template<class ThreadStarts, uint32_t THREADS, uint32_t NNZ_PER_THREAD, typename VALUE_TYPE1, typename VALUE_TYPE2, typename VALUE_TYPE3, typename INDEX_TYPE, typename OFFSET_TYPE, bool ZEROCHECK, bool SCALE, int SORTED_LOAD, int... UNHANDLED, int... HANDLED>
struct Multiplication<ThreadStarts, THREADS, NNZ_PER_THREAD, true, VALUE_TYPE1, VALUE_TYPE2, VALUE_TYPE3, INDEX_TYPE, OFFSET_TYPE, ZEROCHECK, SCALE, SORTED_LOAD, sequence<0, UNHANDLED...>, sequence<HANDLED...>>
{
	...
	template<class COUNTER_SMEM, class THREADINFO_SMEM> __device__ __forceinline__
	static void run(uint32_t flag, const VALUE_TYPE1* matrix, const INDEX_TYPE* inIndex, const OFFSET_TYPE* __restrict offsets, const VALUE_TYPE2* __restrict inVec, VALUE_TYPE3* __restrict outVec, VALUE_TYPE3 scale, uint32_t nnz, COUNTER_SMEM& counterSmem, THREADINFO_SMEM& threadInfoSmem, SMem& smem)
	{
		...
		if (flag == static_cast<uint32_t>(ThreadStarts::Flags::Single))
		{
			...
			if (threadIdx.x == 0)
			{
				...
				atomicAdd(outVec + counterSmem.bStart, res);
			}
		}
		else
...
template<class ThreadStarts, uint32_t THREADS, uint32_t NNZ_PER_THREAD, typename VALUE_TYPE1, typename VALUE_TYPE2, typename VALUE_TYPE3, typename INDEX_TYPE, typename OFFSET_TYPE, bool ZEROCHECK, bool SCALE, int SORTED_LOAD, int... UNHANDLED, int... HANDLED>
struct Multiplication<ThreadStarts, THREADS, NNZ_PER_THREAD, false, VALUE_TYPE1, VALUE_TYPE2, VALUE_TYPE3, INDEX_TYPE, OFFSET_TYPE, ZEROCHECK, SCALE, SORTED_LOAD, sequence<0, UNHANDLED...>, sequence<HANDLED...>>
{
...
template<typename VALUE_TYPE1, typename VALUE_TYPE2, typename VALUE_TYPE3, typename INDEX_TYPE, typename OFFSET_TYPE, uint32_t NNZ_PER_BLOCK, uint32_t THREADS, uint32_t LAUNCHBOUNDS, bool PADDEDLOAD, bool ZERO_CHECK, bool SCALE, int SORTED_LOAD, uint32_t WARPWIDE_FLAG_SWITCH, int... SPECIALIZATIONS>
__launch_bounds__(THREADS, LAUNCHBOUNDS)
__global__ void MultiplyAlong(uint32_t num_non_zeroes, 
                              uint32_t out_size, 
							                uint32_t num_other,
		                          const VALUE_TYPE1* matrix, 
							                const INDEX_TYPE*  inIndex, 
							                const OFFSET_TYPE* __restrict offsets, 
							                const VALUE_TYPE2* __restrict inVec,
		                          VALUE_TYPE3* __restrict outVec, 
							                VALUE_TYPE3 scale,
...
__device__ void sortedResolve(TYPE(&values)[NNZ_PER_THREAD], INDEXTYPE(&col_indices)[NNZ_PER_THREAD], ACCESS_F accessF, COLL_ACCESS_F collAccessF, SEP_FUNCTION sepF)
{
  ...
  #pragma unroll
	...
	sepF();
	INDEXTYPE lower_col = __shfl_up_sync(0xFFFFFFFF,col_indices[NNZ_PER_THREAD - 1], 1);
	bool pivot = lower_col != col_indices[NNZ_PER_THREAD - 1];
	if (__popc(__ballot_sync(0xFFFFFFFF, pivot)) >= 16)
	{
		//atomic out
...
#pragma unroll
		for (int offset = 1; offset < WARP_SIZE; offset *= 2)
		{
			TYPE incoming_res = __shfl_down_sync(0xFFFFFFFF, pivot ? 0.0f : out, offset);
			if (!pivot_right)
				out += incoming_res;
			bool incoming_pivot = __shfl_down_sync(0xFFFFFFFF, pivot || pivot_right, offset);
			pivot_right = pivot_right || incoming_pivot;
...
#pragma unroll
		for (uint32_t offset = 1; offset < WARP_SIZE; offset *= 2)
			out = out + __shfl_down_sync(0xFFFFFFFF, out, offset);
		if (laneid() == 0)
...
void hola_spmv(void* tempmem, size_t& tempmemsize, dDenseVector<T>& res, const dCSR<T>& m, const dDenseVector<T>& v, HolaMode mode, bool transpose, bool padded)
{
	...
	if (padded)
	{
		...
		case HolaMode::SortedFour:
			MultiplyAlong<T, T, T, unsigned int, unsigned int, 4 * blockSize4, blockSize4, HolaLaunchBounds<4,T>::Bounds, true, ZeroCheck, Scale, true, 4 * blockSize4, 0> <<<requiredBlocks, blockSize >>>(m.nnz, m.rows, m.cols, m.data, m.col_ids, m.row_offsets, v.data, res.data, 1.0, reinterpret_cast<uint32_t*>(tempmem));
			break;
		case HolaMode::SortedEight:
			MultiplyAlong<T, T, T, unsigned int, unsigned int, 8 * blockSize8, blockSize8, HolaLaunchBounds<8, T>::Bounds, true, ZeroCheck, Scale, true, 8 * blockSize8, 0> <<<requiredBlocks, blockSize >>>(m.nnz, m.rows, m.cols, m.data, m.col_ids, m.row_offsets, v.data, res.data, 1.0, reinterpret_cast<uint32_t*>(tempmem));
			break;
		case HolaMode::NonSortedFour:
			MultiplyAlong<T, T, T, unsigned int, unsigned int, 4 * blockSize4, blockSize4, HolaLaunchBounds<4, T>::Bounds, true, ZeroCheck, Scale, false, 4 * blockSize4, 0><< <requiredBlocks, blockSize >>>(m.nnz, m.rows, m.cols, m.data, m.col_ids, m.row_offsets, v.data, res.data, 1.0, reinterpret_cast<uint32_t*>(tempmem));
			break;
		case HolaMode::NonSortedEight:
			MultiplyAlong<T, T, T, unsigned int, unsigned int, 8 * blockSize8, blockSize8, HolaLaunchBounds<8, T>::Bounds, true, ZeroCheck, Scale, false, 8 * blockSize8, 0> <<<requiredBlocks, blockSize >>>(m.nnz, m.rows, m.cols, m.data, m.col_ids, m.row_offsets, v.data, res.data, 1.0, reinterpret_cast<uint32_t*>(tempmem));
			break;
		}
	}
	else
	{
		...
		{
		case HolaMode::SortedFour:
			MultiplyAlong<T, T, T, unsigned int, unsigned int, 4 * blockSize4, blockSize4, HolaLaunchBounds<4, T>::Bounds, false, ZeroCheck, Scale, true, 4 * blockSize4, 0><<<requiredBlocks, blockSize>>>(m.nnz, m.rows, m.cols, m.data, m.col_ids, m.row_offsets, v.data, res.data, 1.0, reinterpret_cast<uint32_t*>(tempmem));
			break;
		case HolaMode::SortedEight:
			MultiplyAlong<T, T, T, unsigned int, unsigned int, 8 * blockSize8, blockSize8, HolaLaunchBounds<8, T>::Bounds, false, ZeroCheck, Scale, true, 8 * blockSize8, 0><<<requiredBlocks, blockSize>>>(m.nnz, m.rows, m.cols, m.data, m.col_ids, m.row_offsets, v.data, res.data, 1.0, reinterpret_cast<uint32_t*>(tempmem));
			break;
		case HolaMode::NonSortedFour:
			MultiplyAlong<T, T, T, unsigned int, unsigned int, 4 * blockSize4, blockSize4, HolaLaunchBounds<4, T>::Bounds, false, ZeroCheck, Scale, false, 4 * blockSize4, 0><<<requiredBlocks, blockSize>>>(m.nnz, m.rows, m.cols, m.data, m.col_ids, m.row_offsets, v.data, res.data, 1.0, reinterpret_cast<uint32_t*>(tempmem));
			break;
		case HolaMode::NonSortedEight:
			MultiplyAlong<T, T, T, unsigned int, unsigned int, 8 * blockSize8, blockSize8, HolaLaunchBounds<8, T>::Bounds, false, ZeroCheck, Scale, false, 8 * blockSize8, 0><<<requiredBlocks, blockSize>>>(m.nnz, m.rows, m.cols, m.data, m.col_ids, m.row_offsets, v.data, res.data, 1.0, reinterpret_cast<uint32_t*>(tempmem));
			break;
		}
	}
};
