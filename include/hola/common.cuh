...
template<int Bytes> struct VecLoadTypeImpl;

template<> struct VecLoadTypeImpl<4>
{
	using type = unsigned int;
};
template<> struct VecLoadTypeImpl<8>
{
	using type = uint2;
};
template<> struct VecLoadTypeImpl<16>
{
	using type = uint4;
};

template<typename T, int N> struct VecLoadType
{
...
template<int VecSize, class T, int N> __device__ __forceinline__ 
void warp_load_vectorized(T (&out)[N], const T* in)
{
...
__device__ __forceinline__ void vectorized_to_blocked(T(&data)[N])
{
	...
	for (int j = 0; j < Vecs; ++j)
	{
		...
		#pragma unroll
		for (int i = 0; i < VecSize; ++i)
			data[j*VecSize + i] = __shfl_sync(0xFFFFFFFF, data[j*VecSize + i], shfl_offset);
	}
...
template<class T>
__device__ __forceinline__ void holaswap(T& a, T& b)
{
	T temp = a;
	a = b;
	b = temp;
